...
class WebSocketManager:
    def __init__(self):
        ...
        try:
            ...
            logger.info(
                "[WebSocketManager] No running event loop found during initialization"
            )
    ...
    def _start_background_tasks(self):
        ...
        try:
            ...
            logger.info(
                f"[WebSocketManager] Updated main event loop reference: {self._main_loop}"
            )
            ...
            logger.info(
                "[WebSocketManager] No event loop running, background tasks will start later"
            )
    ...
    async def connect(self, websocket: WebSocket, client_id: str) -> bool:
        ...
        try:
            ...
            self.connection_info[client_id] = WebSocketConnection(
                client_id=client_id, connected_at=datetime.utcnow()
            )
        ...
        try:
            ...
            if client_id in self.connection_info:
                ...
                for run_id in connection.processing_sessions.copy():
                    await self.cancel_processing_session(
                        run_id, f"Client disconnected: {reason}"
                    )
            ...
            logger.error(
                f"[WebSocketManager] Error disconnecting client {client_id}: {e}"
            )
    ...
    async def send_message(self, client_id: str, message_type: str, data: Any) -> bool:
        ...
        if client_id not in self.connections:
            logger.warning(
                f"[WebSocketManager] Client {client_id} not connected; cannot send {message_type}"
            )
            return False
        ...
        try:
            ...
            logger.error(
                f"[WebSocketManager] Error sending {message_type} to {client_id}: {e}"
            )
    ...
    async def broadcast_message(
        self, message_type: str, data: Any, exclude_clients: List[str] = None
    ) -> int:
        ...
        try:
            ...
            try:
                ...
                await self.send_error(
                    client_id, "invalid_json", f"Invalid JSON: {str(e)}"
                )
                return False
            ...
            if not message_type:
                await self.send_error(
                    client_id, "missing_type", "Message type is required"
                )
                return False
            ...
            if message_type in self.message_handlers:
                ...
                try:
                    ...
                    logger.error(
                        f"[WebSocketManager] Handler error for {message_type}: {e}"
                    )
                    await self.send_error(
                        client_id, "handler_error", f"Error processing {message_type}"
                    )
                    return False
            else:
                await self.send_error(
                    client_id, "unknown_type", f"Unknown message type: {message_type}"
                )
                return False

        except Exception as e:
            logger.error(
                f"[WebSocketManager] Error handling message from {client_id}: {e}"
            )
            await self.send_error(
                client_id, "processing_error", "Error processing message"
            )
            return False
    ...
    def create_processing_session(
        ...
        logger.info(
            f"[WebSocketManager] Created processing session {run_id} for client {client_id}"
        )
        return session
    ...
    async def cancel_processing_session(self, run_id: str, reason: str = "Cancelled"):
        ...
        await self.send_message(
            session.client_id,
            "processing_status",
            {"run_id": run_id, "status": "cancelled", "message": reason},
        )
        ...
        logger.info(
            f"[WebSocketManager] Cancelled processing session {run_id}: {reason}"
        )
    ...
    def cleanup_processing_session(self, run_id: str):
        ...
        if run_id in self.processing_sessions:
            ...
            # Remove from client's session list
            if session.client_id in self.connection_info:
                self.connection_info[session.client_id].remove_processing_session(
                    run_id
                )
                ...
                if self.connections:
                    ...
                    for client_id in list(self.connections.keys()):
                        try:
                            ...
                            logger.warning(
                                f"[WebSocketManager] Failed to send heartbeat to {client_id}: {e}"
                            )
    ...
    def get_connection_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_sessions_by_status": {
                status: len(
                    [s for s in self.processing_sessions.values() if s.status == status]
                )
                for status in [
                    "started",
                    "running",
